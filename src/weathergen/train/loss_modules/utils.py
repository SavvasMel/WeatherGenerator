...
def compute_cos_sim_to_prev(
    ...
    Aux (register/class) tokens are dropped and the rest flattened, so each patch token gives
    one value. prev_tokens is detached: the loss flows only through the current forecast step.
